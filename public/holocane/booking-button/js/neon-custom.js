...
(function ($, window, undefined) {
  ...
  $(document).ready(function () {
    ...
    public_vars.$horizontalMenu =
      public_vars.$pageContainer.find("header.navbar");
    public_vars.$sidebarMenu = public_vars.$pageContainer.find(".sidebar-menu");
    public_vars.$mainMenu = public_vars.$sidebarMenu.find("#main-menu");
    public_vars.$mainContent = public_vars.$pageContainer.find(".main-content");
    public_vars.$sidebarUserEnv =
    ...
    public_vars.$sidebarMenu.data(
      "initial-state",
      public_vars.$pageContainer.hasClass("sidebar-collapsed")
        ? "closed"
        : "open"
    );
      ...
      $(".scrollable").each(function (i, el) {
        ...
        $this
          ...
          .slimScroll({
            ...
            alwaysVisible:
              parseInt(attrDefault($this, "autohide", 1), 10) == 1
                ? false
                : true,
          });
      });
    ...
    $("body")
      .on(
        "click",
        '.panel > .panel-heading > .panel-options > a[data-rel="reload"]',
        function (ev) {
          ev.preventDefault();
      ...
      .on(
        "click",
        '.panel > .panel-heading > .panel-options > a[data-rel="close"]',
        function (ev) {
          ev.preventDefault();
          var $this = $(this),
          ...
          t.append(TweenMax.to($panel, 0.2, { css: { scale: 0.95 } }));
          t.append(
            TweenMax.to($panel, 0.5, {
              css: { autoAlpha: 0, transform: "translateX(100px) scale(.95)" },
            })
          );
        }
      )
      .on(
        "click",
        '.panel > .panel-heading > .panel-options > a[data-rel="collapse"]',
        function (ev) {
          ev.preventDefault();
          var $this = $(this),
            $panel = $this.closest(".panel"),
    ...
    $('[data-toggle="popover"]').each(function (i, el) {
      var $this = $(el),
        ...
        popover_class = $this.hasClass("popover-secondary")
          ? "popover-secondary"
          : $this.hasClass("popover-primary")
          ? "popover-primary"
          : $this.hasClass("popover-default")
          ? "popover-default"
          : "";
    ...
    $('[data-toggle="tooltip"]').each(function (i, el) {
      var $this = $(el),
        ...
        popover_class = $this.hasClass("tooltip-secondary")
          ? "tooltip-secondary"
          : $this.hasClass("tooltip-primary")
          ? "tooltip-primary"
          : $this.hasClass("tooltip-default")
          ? "tooltip-default"
          : "";
    ...
    if ($.isFunction($.fn.slider)) {
      $(".slider").each(function (i, el) {
        var $this = $(el),
          ...
          orientation =
            attrDefault($this, "vertical", 0) != 0 ? "vertical" : "horizontal",
        ...
        if (is_range) {
          ...
          $label_1.html(
            (prefix ? prefix : "") + min_val + (postfix ? postfix : "")
          );
          $handles.first().append($label_1);
          $label_2.html(
            (prefix ? prefix : "") + max_val + (postfix ? postfix : "")
          );
          ...
          $this.slider({
            ...
            slide: function (ev, ui) {
              var val =
                (prefix ? prefix : "") + ui.value + (postfix ? postfix : "");
              $label_1.html(val);
        ...
        $this.daterangepicker(opts, function (start, end) {
          ...
          if ($this.hasClass("daterange-inline")) {
            $this
              .find("span")
              .html(
                start.format(drp.format) +
                  drp.separator +
                  end.format(drp.format)
              );
          }
        });
      });
      ...
      $("[data-mask]").each(function (i, el) {
        var $this = $(el),
          ...
          opts = {
            ...
            rightAlignNumerics:
              attrDefault($this, "numericAlign", "left") == "right",
          },
            ...
            TweenLite.to(o, duration / 1000, {
              ...
              onUpdate: function () {
                $num.html(
                  prefix +
                    (format
                      ? numberWithCommas(Math.round(o.curr))
                      : Math.round(o.curr)) +
                    postfix
                );
              },
    ...
    public_vars.$body.on(
      ...
      function (ev) {
        if ($(ev.target).is(".modal")) {
          ...
          tt.append(
            TweenMax.to($modal_dialog, 0.1, {
              css: { scale: 1.1 },
              ease: Expo.easeInOut,
            })
          );
          tt.append(
            TweenMax.to($modal_dialog, 0.3, {
              css: { scale: 1 },
              ease: Back.easeOut,
            })
          );
    ...
    if ($search_results_env.length) {
      var $sr_nav_tabs = $search_results_env.find(".nav-tabs li"),
        $sr_tab_panes = $search_results_env.find(
          ".search-results-panes .search-results-pane"
        );
...
function setup_sidebar_menu() {
  ...
  $items_with_submenu.each(function (i, el) {
    ...
    $link.click(function (ev) {
      ...
      if (!is_multiopen && $this.hasClass(root_level_class)) {
        var close_submenus = public_vars.$mainMenu
          .find("." + root_level_class)
          .not($this)
          .find("> ul");
  ...
  public_vars.$mainMenu
    .find("." + submenu_options.submenu_opened_class + " > ul")
    .addClass("visible");
...
function menu_do_expand($submenu, $this, options) {
  ...
  var props_from = { opacity: 0.2, height: 0, top: -20 },
    props_to = { height: current_height, opacity: 1, top: 0 };
  if (isxs()) {
...
function menu_do_collapse($submenu, $this, options) {
  if (
    public_vars.$pageContainer.hasClass("sidebar-collapsed") &&
    $this.hasClass("root-level")
  ) {
    return;
  }
  $this.removeClass(options.submenu_opened_class);
  TweenMax.to($submenu, options.submenu_open_delay, {
    css: { height: 0, opacity: 0.2 },
    ease: options.submenu_open_easing,
    onUpdate: ps_update,
    onComplete: function () {
  ...
  $items_with_submenu.each(function (i, el) {
    ...
    $link.click(function (ev) {
      if (isxs() || is("tabletscreen")) {
        ev.preventDefault();
        if (!is_multiopen && $this.hasClass(root_level_class)) {
          var close_submenus = $nav_bar_menu
            .find("." + root_level_class)
            .not($this)
            .find("> ul");
          close_submenus.each(function (i, el) {
...
jQuery(public_vars, { hover_index: 4 });
function setup_horizontal_menu_hover($item, $sub) {
  ...
  $item.hoverIntent({
    over: function () {
      ...
      TweenMax.to($sub, del, {
        css: { autoAlpha: 1, transform: "translateX(0px)" },
        ease: ease,
      });
    },
    out: function () {
      ...
      TweenMax.to($sub, del, {
        ...
        onComplete: function () {
          TweenMax.set($sub, {
            css: { transform: "translateX(" + trans_x + "px)" },
          });
...
function blockUI($el) {
  $el.block({
    ...
    overlayCSS: { backgroundColor: "#fff", opacity: 0.3, cursor: "wait" },
  });
}
...
function callback_test() {
  alert(
    "Callback function executed! No. of arguments: " +
      arguments.length +
...
function setCurrentProgressTab($rootwizard, $nav, $tab, $progress, index) {
  ...
  if ($first_tab.hasClass("active")) {
    ...
    if (rtl()) {
      $progress.width(
        $progress.parent().outerWidth(true) -
          $tab.prev().position().left -
          $tab.find("span").width() / 2
      );
    } else {
...
function replaceCheckboxes() {
  var $ = jQuery;
  $(
    ".checkbox-replace:not(.neon-cb-replacement), .radio-replace:not(.neon-cb-replacement)"
  ).each(function (i, el) {
    var $this = $(el),
    ...
    $input
      .on("change", function (ev) {
        if (is_radio) {
          $(
            ".neon-cb-replacement input[type=radio][name='" +
              name +
              "']:not(:checked)"
          )
            .closest(".neon-cb-replacement")
            .removeClass(checked_class);
        }
        if ($input.is(":disabled")) {
          $wrapper.addClass("disabled");
        }
        $this[$input.is(":checked") ? "addClass" : "removeClass"](
          checked_class
        );
      })
      .trigger("change");
...
function init_page_transitions() {
  ...
  for (var i in transitions) {
    ...
    if (public_vars.$body.hasClass(transition_name)) {
      ...
      setTimeout(function () {
        public_vars.$body.removeClass(
          transition_name + " " + transition_name + "-init"
        );
      }, 850);
      return;
    }
...
function show_loading_bar(options) {
  ...
  if ($loading_bar.length == 0) {
    $loading_bar = $(
      '<div class="neon-loading-bar progress-is-hidden"><span data-pct="0"></span></div>'
    );
