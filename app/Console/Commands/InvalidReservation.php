...
class InvalidReservation extends Command
{
    ...
    {
        // Room
        RoomRsvp::whereRaw('reservation_id is null or reservation_id = ?',[''])->where('expired_at', '<', Carbon::now())->delete();
        RoomRsvp::whereRaw('rsvp_payment is null')->where('expired_at', '<', Carbon::now())->update(['rsvp_status' => "Failed"]);

        // Product
        ProductRsvp::whereRaw('reservation_id is null or reservation_id = ?',[''])->where('expired_at', '<', Carbon::now())->delete();
