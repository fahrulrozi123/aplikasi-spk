...
class DeleteReservation extends Command
{
    ...
    {
        ProductRsvp::leftJoin('payment', 'product_rsvp.reservation_id', '=', 'payment.rsvp_id')->whereRaw('rsvp_id is null or reservation_id = ?',[''])->where('expired_at', '<', Carbon::now())->update(['rsvp_status' => "Failed"]);
        RoomRsvp::leftJoin('payment', 'room_rsvp.reservation_id', '=', 'payment.rsvp_id')->whereRaw('rsvp_id is null or reservation_id = ?',[''])->where('expired_at', '<', Carbon::now())->update(['rsvp_status' => "Failed"]);
